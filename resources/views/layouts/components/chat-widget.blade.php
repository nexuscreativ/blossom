...
<div id="blossom-chat" class="blossom-chat-root">
    ...
    <div class="blossom-chat-window" id="blossom-chat-window">
        <div class="blossom-chat-head">
            ...
            <div class="meta">
                <div class="name">{{ $chatName }} Support</div>
    ...
    <script>
        (function () {
            ...
            function loadSession() {
                fetch('{{ route("chat.session") }}', { headers: { 'Accept': 'application/json', 'X-CSRF-TOKEN': CSRF, 'X-Requested-With': 'XMLHttpRequest' }, credentials: 'same-origin' })
                    .then(r => r.json())
                    .then(d => {
                        session = d.session;
