...
<head>
    ...
    <meta name="description" content="@yield('metaDescription', $siteDescription)">
    ...
    {{-- Main CSS --}}
    <link rel="stylesheet" href="{{ asset('assets/css/blossom.css') }}">

    {{-- Favicon --}}
    <link rel="icon" type="image/png" href="{{ asset('assets/blossom-logo.png') }}">
...
<body class="font-body bg-page text-primary antialiased" x-data="{ mobileMenuOpen: false, theme: localStorage.getItem('theme') || 'light' }" :data-theme="theme">
    ...
    <div id="preloader" class="fixed inset-0 z-[9999] flex items-center justify-center bg-ink transition-opacity duration-700">
        <div class="text-center">
            <div class="preloader-logo mb-4 opacity-0" id="preloader-logo">
                <img src="{{ asset('assets/blossom-logo.png') }}" alt="BLOSSOM Logo" class="h-20 w-auto object-contain mx-auto">
            </div>
            <div class="preloader-tagline font-ui text-ash text-sm tracking-[0.3em] uppercase opacity-0" id="preloader-tagline">{{ $siteTagline }}</div>
    ...
    {{-- Newsletter Modal --}}
    @include('layouts.components.newsletter-modal')

    {{-- Support Chat Widget --}}
    @include('layouts.components.chat-widget')
