...
<head>
    ...
    <title>Installer &middot; {{ config('app.name', 'Blossom') }}</title>
    <style>
        :root {
            --ink: #1c1917;
            --onion: #14532d;
...
<body>
    <div class="card">
        ...
        <h1>Welcome</h1>
        <p>Thanks for choosing {{ config('app.name', 'Blossom') }}. Before we get started, let's make sure your server is ready.</p>

        <h2>Server Requirements</h2>
        <ul class="checks">
            @foreach ($requirements as $label => $check)
                <li class="{{ $check['ok'] ? 'ok' : ($check['blocking'] ? 'fail' : 'warn') }}">{{ $label }}</li>
            @endforeach
        </ul>
        ...
        <ul class="checks">
            @foreach ($writable as $label => $ok)
                <li class="{{ $ok ? 'ok' : 'warn' }}">{{ $label }} <small>(must be writable)</small></li>
            @endforeach
        </ul>
            ...
            <div class="section">
                ...
                <form method="POST" action="{{ route('install.run') }}">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="app_name" name="app_name" value="{{ old('app_name', 'BLOSSOM') }}" required>
                    </div>

                    <div class="form-group">
                        <label for="app_url">Site URL <small>(your domain)</small></label>
                        <input type="url" id="app_url" name="app_url" value="{{ old('app_url', url('/')) }}" placeholder="https://example.com" required>
                    </div>

                    <div class="form-group">
                        <label for="db_connection">Database Driver</label>
                        <select id="db_connection" name="db_connection" onchange="toggleDbFields()">
                            <option value="sqlite" {{ old('db_connection', 'sqlite') === 'sqlite' ? 'selected' : '' }}>SQLite (recommended — zero config)</option>
                    ...
                    <div id="sqlite-fields">
                        <div class="form-group">
                            ...
                            <input type="text" id="db_database_sqlite" name="db_database" value="{{ old('db_database', 'database/database.sqlite') }}">
                    ...
                    <div id="server-db-fields" style="display:none">
                        <div class="grid-3">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" id="db_database_server" name="db_database" value="{{ old('db_database') }}">
                            </div>
                        </div>
                        <div class="grid-2">
                            <div class="form-group">
                    ...
                    <div class="section">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="admin_name" name="admin_name" value="{{ old('admin_name', 'Admin') }}" required>
                        </div>
                        <div class="form-group">
                            <label for="admin_email">Admin Email</label>
                            <input type="email" id="admin_email" name="admin_email" value="{{ old('admin_email') }}" placeholder="you@example.com" required>
                        </div>
                        <div class="form-group">
                    ...
                    <div class="section">
                        ...
                        <div class="grid-2">
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" id="mail_port" name="mail_port" value="{{ old('mail_port') }}">
                            </div>
                        </div>
                        <div class="grid-2">
                            <div class="form-group">
