            ...
            @foreach($perks as $perk)
                <div class="bg-white rounded-2xl p-8 border border-silver hover:shadow-lg hover:shadow-sean/5 transition-all duration-500 text-center">
                    <div class="w-14 h-14 mx-auto rounded-xl bg-sean/5 border border-sean/10 flex items-center justify-center mb-5">
                        <svg class="w-6 h-6 text-sean" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">{!! $perk['icon'] !!}</svg>
                    </div>
                    <h3 class="font-display text-xl font-bold text-ink mb-3">{{ $perk['title'] }}</h3>
                    <p class="font-body text-sm text-secondary leading-relaxed">{{ $perk['desc'] }}</p>
                </div>
            @endforeach
        </div>
    </div>
            ...
            @foreach($roles as $role)
                <div class="bg-white rounded-2xl p-6 border border-silver flex items-center justify-between gap-6 hover:shadow-lg hover:shadow-onion/5 transition-all duration-500">
                    <div>
                        ...
                        <span class="font-ui text-xs text-muted uppercase tracking-wider">{{ $role['type'] }}</span>
                    </div>
                    <a href="{{ route('contact') }}" class="btn-ghost w-full !w-auto !px-6 !py-2.5 !min-h-0">Apply Now</a>
                </div>
            @endforeach
        </div>
