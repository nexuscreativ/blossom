...
<section class="hero-section">
    <div class="hero-image-wrapper">
        <img src="{{ asset('assets/hero-family.jpg') }}"
    ...
    <div class="hero-content">
        <div class="max-w-3xl">
            ...
            <div class="hero-cta flex flex-wrap gap-4">
                <a href="{{ route('articles.show', 'the-nzem-berom-festival-where-drums-speak-the-language-of-ancestors') }}" class="btn-primary">
                    Read Article
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3"/>
                    </svg>
                </a>
                <a href="{{ route('articles.index') }}" class="btn-ghost">Explore More Stories</a>
            </div>
        </div>
    </div>
    ...
    <div class="container-blossom">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 stagger-children">
            ...
            @foreach($picks as $pick)
                <a href="{{ route('articles.show', $pick['slug']) }}" class="article-card group">
                    <div class="article-card-image">
                        <img src="{{ $pick['img'] }}" alt="{{ $pick['title'] }}" loading="lazy">
                        <div class="absolute top-4 left-4 z-10">
                            <span class="category-pill category-pill--{{ $pick['catColor'] }}">{{ $pick['cat'] }}</span>
                        </div>
                        @if($pick['premium'])
                    ...
                    <div class="p-6">
                        ...
                        <p class="font-body text-secondary text-sm leading-relaxed mb-4">{{ $pick['excerpt'] }}</p>
    ...
    <div class="container-blossom">
        ...
        <div class="trending-strip flex gap-4 pb-4">
            ...
            @foreach($trending as $item)
                <a href="{{ route('articles.index') }}" class="trending-item">
                    <span class="trending-number">{{ $item['num'] }}</span>
                    <div>
                        <span class="font-ui text-[10px] font-semibold tracking-wider uppercase text-onion">{{ $item['cat'] }}</span>
                        <p class="font-ui text-sm font-medium text-ink leading-snug mt-0.5">{{ $item['title'] }}</p>
                    </div>
                </a>
            @endforeach
        </div>
        ...
        <div class="flex gap-1 overflow-x-auto pb-4 mb-10 scrollbar-hide reveal">
            ...
            @foreach($tabs as $tab)
                <button @click="activeTab = '{{ $tab['id'] }}'"
                        ...
                        :class="activeTab === '{{ $tab['id'] }}' ? 'bg-onion text-white shadow-onion' : 'bg-white text-graphite hover:text-onion border border-silver'">
                    {{ $tab['label'] }}
                </button>
            @endforeach
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div class="lg:col-span-5 flex flex-col gap-4 reveal-right">
                ...
                @foreach($sideArticles as $article)
                    <a href="{{ route('articles.show', $article['slug']) }}" class="flex gap-4 p-4 rounded-xl bg-white border border-silver hover:border-onion hover:shadow-md transition-all duration-300 group">
                        <img src="{{ $article['img'] }}" alt="" class="w-20 h-20 rounded-lg object-cover flex-shrink-0" loading="lazy">
                        <div class="flex flex-col justify-center min-w-0">
                            <span class="font-ui text-[10px] font-semibold tracking-wider uppercase text-onion">{{ $article['cat'] }}</span>
                            ...
                            <span class="font-ui text-xs text-muted mt-1">{{ $article['time'] }} read</span>
                        </div>
                    </a>
                @endforeach
            </div>
...
<section class="py-20 lg:py-28 bg-pearl">
    <div class="container-blossom">
        <div class="flex items-end justify-between mb-10">
            ...
            <a href="{{ route('listings.index') }}" class="hidden md:flex items-center gap-2 font-ui text-sm font-medium text-onion hover:text-orange transition-colors">
                View All
                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3"/>
                </svg>
            ...
            @foreach($listings as $listing)
                <a href="{{ route('listings.show', $listing['slug']) }}" class="listing-card {{ $listing['featured'] ? 'listing-card--featured' : '' }} group">
                    ...
                    <div class="p-5">
                        ...
                        <h3 class="font-display text-lg font-bold text-ink mt-1 group-hover:text-orange transition-colors">{{ $listing['name'] }}</h3>
                        @if($listing['featured'])
...
<section class="py-20 lg:py-28">
    <div class="container-blossom">
        <div class="flex items-end justify-between mb-10">
            ...
            <a href="{{ route('events.index') }}" class="hidden md:flex items-center gap-2 font-ui text-sm font-medium text-sean hover:text-sean-dark transition-colors">
                View All Events
                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3"/>
                </svg>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-4xl mx-auto stagger-children">
            ...
            <div class="pricing-card text-center">
                ...
                <ul class="space-y-3 text-left mb-8">
                    @foreach(['5 articles per month', 'Basic event listings', 'Newsletter access', 'Community (read-only)'] as $feature)
                ...
                <a href="{{ route('login') }}" class="btn-ghost w-full">Get Started</a>
            </div>
            ...
            <div class="pricing-card pricing-card--featured text-center">
                ...
                <ul class="space-y-3 text-left mb-8">
                    @foreach(['Unlimited articles', 'Full premium content', 'Community access', 'Event early access', 'Ad-free experience'] as $feature)
                ...
                <a href="{{ route('pricing') }}" class="btn-primary w-full">Subscribe Now</a>
            </div>
            ...
            <div class="pricing-card text-center">
                ...
                <ul class="space-y-3 text-left mb-8">
                    @foreach(['Everything in Insider', 'Priority event tickets', '1 free listing/year', 'Patron badge', 'Editorial access'] as $feature)
                        <li class="flex items-start gap-2 font-ui text-sm text-secondary">
                ...
                <a href="{{ route('pricing') }}" class="btn-ghost w-full">Become a Patron</a>
            </div>
        </div>
    </div>
</section>
